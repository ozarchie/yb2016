...
Serial.print("\n");
//  Serial.print("\ne2p >>");
  for (i=E2P_TDS1Value_Low;i<=E2P_NutePump1_OFF;i+=5){
    printflsp(i, eepromReadFloat(i));
  }
...
Serial.print("\n");
//  Serial.print("\ne2p >>");
  for (i=E2P_TDS2Value_Low;i<=E2P_NutePump2_OFF;i+=5){
    printflsp(i, eepromReadFloat(i));
  }
...
printcrcError("Relay State ", EEPROM.read(E2P_CRC8_RelayState), e2p_crc8(E2P_Relay_State, MAXRELAYS*2), MAXRELAYS*2);
    ...
    printcrcError("Light Value ", EEPROM.read(E2P_CRC8_LightV), e2p_crc8(E2P_LightValue_Low, 8), 8);
